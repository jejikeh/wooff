...
namespace Wooff.Examples.Entities
{
    ...
    public class Dog : Entity<Information, Transform, Speaker>
    {
        public void WhoAmIm()
        {
            ...
            var transform = GetFirstNullable<Transform>();
            Console.WriteLine($"TRANSFORM COMPONENT\nX: {transform?.X}\tY: {transform?.Y}\tZ: {transform?.Z}");

            var speaker = GetFirstNullable<Speaker>();
            Console.WriteLine($"SPEAKER COMPONENT\nSpeak: {speaker?.Message}");
            speaker?.Speak();
            Console.WriteLine("--- --- --- --- --- --- ---\n");
        }

        public override IInitable<Information, Transform, Speaker> Init(Information dataT, Transform dataT1,
            Speaker dataT2)
        {
            Add(dataT);
            Add(dataT1);
    ...
    {
        public static Dog AddDog(this IContext<IEntity> context, InformationData informationData,
            SpeakerData speakerData,
            params float[] transformData)
        {
            var dog = context.Add<Dog, Information, InformationData, Transform, float[], Speaker, SpeakerData>(
                informationData, transformData, speakerData);
            return dog;
        }
    }
}